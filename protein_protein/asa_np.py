...
def calculate_asa_np(pdb, probe, n_sphere_point=960):
    ...
    pointsname=pdb.getNames()
    residuesname = pdb.getResnames()
    residuesNumber = pdb.getResnums()
    chainname=pdb.getChids()
    print 'getChid is %s ' % chainname
    ...
    for i in xrange(0,len(pdb)):
        ...
        chainname_i=chainname[i]
        point_i = points[i, :]
        pointname_i = pointsname[i]
        residuename_i=residuesname[i]
        residuenum_i = residuesNumber[i]
        ...
        areas[i] = area
        names.append(pointname_i)
        residuenames.append(residuename_i)
        residuenums.append(residuenum_i)
        chainnames.append(chainname_i)
        #areas.append(area)

    return (areas,names,residuenames,residuenums,chainnames)
...
def main():
    ...
    parser.add_argument('-o1', action='store', required=True, dest='outfile1',
                    help='output directory to store the results')
    parser.add_argument('-s', action='store', required=False, dest='n_sphere',
                    help='number of points used in generating the spherical dot-density for the calculation')
    input=parser.parse_args()


    #mol = molecule.Molecule(args[0])
    #pdb = molecule.Molecule('dimers/1R0R.pdb')
    pdbfile=os.getcwd()+'/interface_analyzer/pdbdata/'+input.pdbfile
    ...
    #for n in n_sphere:
    n = float(input.n_sphere)
    asas,names,residuena,residuenu,chainname = calculate_asa_np(pdb, 1.4, n)
    data.append(asas)
    name.append(names)
    dirname=os.getcwd()+'/interface_analyzer/'+input.outfile
    dirname1=os.getcwd()+'/interface_analyzer/'+input.outfile1
    f_test = open(dirname,'w')
    f_result=open(dirname1,'w')
    #c = csv.writer(f_test)
    for i in xrange(len(data)):
        sumasa=0
        for j in xrange(len(pdb)):
            f_test.write('%s:' % chainname[j])
            f_test.write('%s' % residuenu[j])
            f_test.write(':')
            f_test.write(residuena[j])
            f_test.write(':')
            f_test.write(name[i][j])
            f_test.write(':')
            f_test.write('%s' % data[i][j])
            if(j==0):
                sumasa+=data[i][j]
            elif residuenu[j]==residuenu[j-1] and residuena[j]==residuena[j-1]:
                if j+1<len(pdb) and residuenu[j+1]==residuenu[j]:
                    sumasa+=data[i][j]
                else:
                    sumasa+=data[i][j]
                    f_test.write(':sum:%s' % sumasa)
                    #f_test.write(''sumasa)
                    f_result.write('%s:'% chainname[j]+'%s'% residuenu[j]+':'+residuena[j]+':sum:%s'% sumasa)
                    f_result.write('\n')
                    sumasa=0
            else:
                sumasa+=data[i][j] 
    ...
    f_test.close()
    f_result.close()
    print "the result has been stored in file of %s " % dirname1
    #if len(args) > 1:
