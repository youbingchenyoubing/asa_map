...
def caculateasa(file1,file2,file3='new.txt'):
    ...
    for line in file_1:
        ...
        lenfile_1+=1
    file_1.close()
    file_2=open(file2) #complex's value of sum of asa
    file_2_load = []
    lenfile_2=0
    for line in file_2:
        line=line.strip('\n')
        loadcontains=line.split(':',4)
        if loadcontains[0]== file_1_load[lenfile_2][0] and loadcontains[1] ==file_1_load[lenfile_2][1]: #chain id and resi num 
    ...
    for i in xrange(0,lenfile_2):
        if float(file_1_load[i][4])==0 and float(file_2_load[i][4])==0:
            ...
            resultasa=float(file_1_load[i][4])-float(file_2_load[i][4])
